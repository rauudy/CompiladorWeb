...
class IncrementExpr implements Expression {
    ...
    interpret(ctx: Context) {
        ...
        if (typeof curValue === 'object' && curValue !== null) {
            ...
            if ('value' in curValue && typeof curValue.value === 'number') {
                ...
                throw new Error(`Cannot increment non-numeric value: ${this.identifier} is of type ${typeof curValue}`);
            }
        ...
        if (isNaN(currentValue)) {
            throw new Error(`Cannot increment non-numeric value: ${this.identifier} is not a valid number`);
        }
